  ...
  <ContextMenu @contextmenu.prevent>
    <ContextMenuTrigger>
      <div
        ...
        @click.stop="onClickFlow"
        @dblclick="onOpenWorkflow"
      >
        <div
    ...
    <ContextMenuContent class="w-44">
      <ContextMenuItem @click="onOpenWorkflow">
...
const thumbnail = computed(() => {
  const data = props.flow.thumbnail?.data
  return data
...
function onOpenWorkflow() {
  router.push({ name: 'Flow', params: { id: props.flow.id } })
}
...
function onRenamed() {
  useEventBus('Flows::Refresh').emit()
}
